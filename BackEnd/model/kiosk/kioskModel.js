...
const searchUser = async (req, res) => {
  ...
  const query = mybatisMapper.getStatement('kiosk', 'searchuser', param, format);
  conn.query(query, (err, results) => {
    ...
    if (results.length === 0) {
      ...
      const query2 = mybatisMapper.getStatement('kiosk', 'todayCheck', param, format);
      conn.query(query2, (err2, results2) => {
        if (err2) console.log(err2);
        console.log(results2);
        if (results2.length === 0) {
          console.log('출석체크 X');
          const query3 = mybatisMapper.getStatement('kiosk', 'attendanceEntry', param, format);
          ...
          console.log('출석체크 o');
          const query4 = mybatisMapper.getStatement('kiosk', 'attendanceExit', param, format);
...
const equipmentMonthData = async (req, res) => {
  ...
  console.log(param);
  const format = { language: 'sql', indent: '' };
  const query = mybatisMapper.getStatement('kiosk', 'equipMonthData', param, format);
  conn.query(query, (err, results) => {
    if (err) console.log(err);
    console.log(results);
